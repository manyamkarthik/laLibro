...
export const Checkout = ({ setCheckout }) => {
  ...
  const [data, setData] = useState({ name: "", email: "", id: "" }); // Provide default values
  ...
  async function handleSubmit(event) {
    ...
    const d=await createOrder(cartList,total,data)
    
    clearCart();
    navigate("/order-summary",{state:{data:d,status:true}});
    }
    catch(error){
